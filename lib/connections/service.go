...
// getProgressiveDialTimeoutForAddress returns an adaptive dial timeout for a specific address
// based on connection history
func getProgressiveDialTimeoutForAddress(address string) time.Duration {
	if globalService != nil {
		return globalService.adaptiveTimeouts.getProgressiveDialTimeout(address)
	}
	// Fallback to default timeout if service not available
	return 20 * time.Second
}

// recordConnectionFailureForAddress records a connection failure for adaptive timeout tracking
func recordConnectionFailureForAddress(address string) {
	if globalService != nil {
		globalService.adaptiveTimeouts.recordConnectionFailure(address)
	}
}

// recordConnectionSuccessForAddress records a successful connection for adaptive timeout tracking
func recordConnectionSuccessForAddress(address string) {
	if globalService != nil {
		globalService.adaptiveTimeouts.recordConnectionSuccess(address)
	}
}
...
// tlsTimedHandshake performs a TLS handshake with adaptive timeouts
// When called from contexts where service instance is not available, uses global adaptive timeouts
func tlsTimedHandshake(tc *tls.Conn, adaptiveTimeouts ...*adaptiveTimeouts) error {
	// Use provided adaptive timeouts or fall back to global tracker
	timeoutTracker := globalAdaptiveTimeouts
	if len(adaptiveTimeouts) > 0 && adaptiveTimeouts[0] != nil {
		timeoutTracker = adaptiveTimeouts[0]
	}
	
	// Use adaptive timeout instead of fixed timeout
	timeout := timeoutTracker.calculateAdaptiveTLSHandshakeTimeout()
	tc.SetDeadline(time.Now().Add(timeout))
	defer tc.SetDeadline(time.Time{})
	return tc.Handshake()
}

// calculateAdaptiveConnectionLoopSleep calculates adaptive sleep time for connection loop
// based on connection success rates
func (at *adaptiveTimeouts) calculateAdaptiveConnectionLoopSleep() time.Duration {
	at.mut.RLock()
	defer at.mut.RUnlock()
	
	// Start with standard sleep time
	sleep := stdConnectionLoopSleep
	
	// Adjust based on success rate
	if at.connectionSuccessRate < 0.3 {
		// Poor success rate, increase sleep time to reduce load
		sleep = sleep * 2
	} else if at.connectionSuccessRate > 0.8 {
		// Good success rate, can decrease sleep time for faster reconnects
		sleep = sleep / 2
	}
...
// recordConnectionFailure records a connection failure for a specific address
// to track problematic connections
func (at *adaptiveTimeouts) recordConnectionFailure(address string) {
	at.mut.Lock()
	defer at.mut.Unlock()
	
	// Increment failure count for this address
	at.problematicConnections[address]++
	
	// Clean up old entries to prevent memory leaks
	// Remove addresses with no recent failures
	if at.problematicConnections[address] == 0 {
		delete(at.problematicConnections, address)
	}
}

// getProgressiveDialTimeout calculates an adaptive dial timeout based on
// the number of failures for a specific address
func (at *adaptiveTimeouts) getProgressiveDialTimeout(address string) time.Duration {
	at.mut.RLock()
	defer at.mut.RUnlock()
	...
	// Get failure count for this address
	failures := at.problematicConnections[address]
	
	// Increase timeout progressively based on failure count
	// Each failure increases timeout by 50%, up to a maximum of 5x the base
	multiplier := 1.0 + (float64(failures) * 0.5)
	if multiplier > 5.0 {
		multiplier = 5.0
	}
...
// recordConnectionSuccess records a successful connection to an address
// to reduce its failure count
func (at *adaptiveTimeouts) recordConnectionSuccess(address string) {
	at.mut.Lock()
	defer at.mut.Unlock()
	
	// Reduce failure count for this address (but don't go negative)
	if at.problematicConnections[address] > 0 {
		at.problematicConnections[address]--
	}
...
func (at *adaptiveTimeouts) updateConnectionSuccessRate(success bool) {
	...
	if time.Since(at.lastAdjustment) > 30*time.Second {
		// Increase timeout for poor success rates, decrease for good success rates
		if at.connectionSuccessRate < 0.5 {
			// Poor success rate, increase timeout
			at.tlsHandshakeTimeout = time.Duration(float64(at.tlsHandshakeTimeout) * 1.2)
		} else if at.connectionSuccessRate > 0.9 {
			// Good success rate, can decrease timeout
			at.tlsHandshakeTimeout = time.Duration(float64(at.tlsHandshakeTimeout) * 0.9)
		}
